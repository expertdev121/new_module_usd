...
export async function POST(request: NextRequest) {
  try {
    ...
    // Execute query
    const results = await db.execute(sql.raw(baseQuerySQL));
    ...
    const pdfBuffer = generateReportPDF({
      ...
      subtitle: `${reportType.toUpperCase()} Report`,
      data: pdfData,
    ...
    return new NextResponse(new Uint8Array(pdfBuffer), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="${generateReportFilename(`${reportType}`)}"`,
      },
    });

  } catch (error) {
    console.error('Error generating LYBUNT/SYBUNT PDF:', error);
