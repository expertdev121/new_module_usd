...
export default function UpcomingExpectedPaymentsReportsPage() {
  ...
  const generateReport = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `upcoming-expected-payments-${new Date().toISOString().split('T')[0]}.csv`;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          View expected payments for pledges with outstanding balances where previous payments were made by card
        </p>
      </div>
      ...
      {!loading && reportData.length === 0 && !initialLoad && (
        <div className="text-center py-8 text-muted-foreground">
          No upcoming expected payments found.
        </div>
      )}
    </div>
  );
