...
const TAG_NAME = 'KTC';
const CSV_PATH = path.resolve(__dirname, '../data/YLA Contacts/KTC.csv');
...
interface Row {
  'First Name': string;
  'Last Name': string;
  'Preferred Address Line 1': string;
  'Preferred City': string;
  'Preferred State': string;
  'Preferred ZIP': string;
  Tag: string;
  location: string;
}
...
function buildAddress(r: Row): string {
  const line = (r['Preferred Address Line 1'] || '').trim();
  const city = (r['Preferred City'] || '').trim();
  const state = (r['Preferred State'] || '').trim();
  const zip = normalizeZip(r['Preferred ZIP']);
...
function parseCSV(): Row[] {
  ...
  return parsed.data.filter(
    (r) => (r['First Name'] || '').trim() && (r['Last Name'] || '').trim()
  );
}
...
async function main() {
  ...
  // 1. Bulk fetch existing contacts for this location
  ...
  // 2. Classify CSV rows into updates vs inserts
  ...
  // 3. Batch updates — each row has a different address, so run them concurrently in chunks
  ...
  // 4. Batch inserts of new contacts, capturing IDs
  ...
  // 5. Link KTC tag to ALL processed contacts (existing + newly inserted)
  let tagsLinked = 0;
  if (!DRY_RUN && tagId > 0) {
